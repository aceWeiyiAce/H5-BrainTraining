...
const WORD_ANSWER_MAX_LENGTH = 35;
...
function wordPracticeMarkup(unit, session, word) {
  ...
  const showAnswerInput = !completed;
  ...
  return `
    ...
    <div class="word-answer-wrap ${showAnswerInput ? "" : "hidden"}" data-answer-box>
      <input class="word-answer-input" data-word-answer type="text" inputmode="text" pattern="[A-Za-z]*" maxlength="${WORD_ANSWER_MAX_LENGTH}" autocapitalize="none" autocomplete="off" autocorrect="off" spellcheck="false" lang="en" aria-label="填写完整单词" style="--answer-width: ${answerWidth}px" value="${answer}" />
    </div>
    <div class="button-stack">
...
function bindWordPractice(unit, session, word) {
  ...
  if (answerInput) {
    answerInput.addEventListener("pointerdown", (event) => {
      if (session.phase !== "memory") return;
    ...
    answerInput.addEventListener("beforeinput", (event) => {
      ...
      if (event.inputType === "insertText" || event.inputType === "insertFromPaste") {
        ...
        event.preventDefault();
        insertAnswerText(answerInput, session, letters, WORD_ANSWER_MAX_LENGTH);
      }
    });
    answerInput.addEventListener("input", () => {
      if (session.phase !== "filling") return;
      const cleaned = onlyLetters(answerInput.value).slice(0, WORD_ANSWER_MAX_LENGTH);
